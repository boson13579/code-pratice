...
class Point {
   ...
   public:
    ...
    void display() {
        cout << "(";
        for (int i = 0; i < dim - 1; i++) cout << val[i] << ",";
        cout << val[dim - 1] << ")" << endl;
    }
...
Point ::Point(int a) {
    this->dim = a;
    val = new double[a];
    for (int i = 0; i < sizeof(val); i++) val[i] = 0.0;
    count++;
}
...
Point ::Point(const Point &a) {
    ...
    for (int i = 0; i < this->getDim(); i++) this->setVal(i, a.getVal(i));
    count++;
}
...
void Point ::setVal(int a, double b) { this->val[a] = b; }

double Point ::getVal(int a) const { return this->val[a]; }

ostream &operator<<(ostream &s, Point &p) {
    s << "(";
    for (int i = 0; i < p.getDim() - 1; i++) s << p.getVal(i) << ",";
...
Point operator+(Point &p, Point &p2) {
    Point temp;
    if (p.getDim() != p2.getDim()) return temp;
...
const Point &operator+=(Point &p, Point &p2) {
    if (p.getDim() != p2.getDim()) return p;
    p = p + p2;
    return p;
}

Point operator++(Point &a) {
    for (int i = 0; i < a.getDim(); i++) a.setVal(i, a.getVal(i) + 1);
    return a;
}

Point operator*(Point &a, double b) {
    for (int i = 0; i < a.getDim(); i++) a.setVal(i, a.getVal(i) * b);
    return a;
}

double operator-(const Point p, const Point p2) {
    if (p.getDim() != p2.getDim()) return 0.0;
    double ans = 0;
    for (int i = 0; i < p.getDim(); i++) {
...
bool operator==(const Point &a, const Point &b) {
    if (a.getDim() != b.getDim()) return false;
    for (int i = 0; i < a.getDim(); i++) {
        if (a.getVal(i) != b.getVal(i)) return false;
    }
    return true;
}
...
Point operator!(const Point &a) {
    Point temp;
    temp = a;
    for (int i = 0; i < temp.getDim(); i++) temp.setVal(i, -temp.getVal(i));
    return temp;
}
