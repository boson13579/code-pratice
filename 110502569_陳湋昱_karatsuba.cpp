...
struct Big:vector<int> {
	...
	friend Big operator*(const Big &lhs, const Big &rhs) {
		...
		Big L_low(lhs.begin(), lhs.begin() + mid), L_hei(lhs.begin() + mid, lhs.end());
		Big R_low(rhs.begin(), rhs.begin() + mid), R_hei(rhs.begin() + mid, rhs.end());

		Big A = (L_low * R_low);
		Big C = (L_hei * R_hei);
		Big B = (L_low + L_hei) * (R_low + R_hei) - A - C;

		if (!B.iszero()) A = A + B.shift(mid);
		if (!C.iszero()) A = A + C.shift(mid + mid);
